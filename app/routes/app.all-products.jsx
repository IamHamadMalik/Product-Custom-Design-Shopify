...
export const loader = async ({ request }) => {
  ...
  const tagsSuccess = url.searchParams.get("tagsSuccess") === "1"; // New query param
  ...
  return json({
    ...
    success,
    tagsSuccess, // Pass tagsSuccess to component
    editedProducts,
    shop: session.shop,
  });
};
...
export const action = async ({ request }) => {
  ...
  return redirect(`/app/bulk-editor-config?ids=${ids.join(",")}`);
};
...
export default function BulkProducts() {
  const {
    ...
    filters,
    success,
    tagsSuccess, // Receive tagsSuccess from loader
    editedProducts,
    shop,
  } = useLoaderData();
  ...
  const [showTagsRefreshSuccess, setShowTagsRefreshSuccess] = useState(tagsSuccess); // Initialize with tagsSuccess
  ...
  const handleRefreshTags = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        params.set("tagsSuccess", "1"); // Add tagsSuccess query param
        submit(params, { replace: true });
      } else {
  ...
  const handleDismissSuccess = () => {
    ...
    setShowTagsRefreshSuccess(false); // Also dismiss tags success message
    const params = new URLSearchParams(window.location.search);
    params.delete("success");
    params.delete("edited_ids");
    params.delete("tagsSuccess"); // Clear tagsSuccess param
    submit(params, { replace: true });
  };
